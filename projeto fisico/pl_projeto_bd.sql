...
CREATE OR REPLACE PROCEDURE PROC_CADASTRAR_HOTEL (NUM_HOTEL NUMBER, CEP_HOTEL VARCHAR2) IS
BEGIN
...
CREATE OR REPLACE PROCEDURE PROC_CADASTRAR_FLAT (COD_HOTEL NUMBER, NUM_FLAT NUMBER, STATUS_FLAT VARCHAR2) IS
BEGIN
...
CREATE OR REPLACE PROCEDURE PROC_CADASTRAR_SERVICOS_ADICIONAIS (NOME_SERVICO VARCHAR2, CUSTO_ADICIONAL VARCHAR2) IS
BEGIN
...
CREATE OR REPLACE PROCEDURE PROC_CADASTRAR_FUNCIONARIO (CPF_FUNCIONARIO VARCHAR2, NOME_FUNCIONARIO VARCHAR2, CARGO_FUNCIONARIO VARCHAR2, CPF_CHEFE VARCHAR2, COD_HOTEL NUMBER) IS
BEGIN
...
CREATE OR REPLACE PROCEDURE PROC_CADASTRAR_PROMOCAO (DESCONTO VARCHAR2) IS
BEGIN
...
BEGIN
    ...
    -- Atualiza "status" do responsável no hotel, ou seja, registra a ocupação
...
CREATE OR REPLACE PROC_INSERIR_RESPONSAVEL_NO_PAGAMENTO (V_CPF_NEW_RESPONSAVEL VARCHAR2, V_NUM_FLAT NUMBER, V_COD_HOTEL NUMBER, V_DT_INIC DATE) IS
DECLARE 
    V_COD_HOSPEDAGEM NUMBER := NULL;
    V_COUNT NUMBER := NULL;
BEGIN  
    -- busca cod_hospedagem do responsavel
    SELECT COD_HOSPEDAGENS INTO V_COD_HOSPEDAGEM FROM HOSPEDAGENS
    WHERE NUM_FLAT = V_NUM_FLAT AND COD_HOTEL = V_COD_HOTEL AND V_DT_INIC = V_DT_INIC
    ...
    -- Insere o responsável no pagamento da hospedagem
...
BEGIN  
    ...
    -- Adiciona o valor na conta do responsável
    UPDATE RESPONSAVEL
    SET CONTA_RESPONSAVEL = TO_CHAR(TO_NUMBER(CONTA_RESPONSAVEL) + TO_NUMBER(V_CUSTO_ADICIONAL))
