...
-- Exibir crachás que já estão sendo usado e seu respectivo funcionário
...
SELECT H.NUM_HOTEL, F.CPF_FUNCIONARIO, F.NOME_FUNCIONARIO, F.CARGO_FUNCIONARIO
FROM FUNCIONARIO F
...
SELECT H.NOME_HOSPEDE, COUNT(*) AS QTD
FROM HOSPEDE H
GROUP BY H.NOME_HOSPEDE
HAVING COUNT(*) = (
...
WHERE (RPH.COD_HOSPEDAGENS, RPH.COD_PROMOCAO) IN (
